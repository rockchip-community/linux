...
#define RKISP1_DRIVER_NAME			"rkisp30"
...
struct rkisp1_dmarx_chan {
	struct rkisp1_device *rkisp1;
	struct rkisp1_vdev_node vnode;
	spinlock_t buf_lock;
	struct list_head buf_queue;
	struct rkisp1_buffer *curr_buf;
	const struct rkisp1_dmarx_fmt *fmts;
	unsigned int fmt_cnt;
	const struct rkisp1_dmarx_fmt *fmt;
	struct v4l2_pix_format_mplane pix;
	const struct rkisp1_rawrd_cfg *cfg;
	enum rkisp1_rawrd_id id;
	bool streaming;
};

struct rkisp1_dmarx {
	struct rkisp1_dmarx_chan chan[RKISP1_RAWRD_MAX];
