...
static const struct rkisp1_capture_config rkisp1_capture_config_mp = {
	...
	.mi = {
		...
		.y_base_ad_shd =	RKISP1_CIF_MI_MP_Y_BASE_AD_SHD,
...
static const struct rkisp1_capture_config rkisp1_capture_config_sp = {
	...
	.mi = {
		...
		.y_base_ad_shd =	RKISP1_CIF_MI_SP_Y_BASE_AD_SHD,
...
static void rkisp1_mp_config(struct rkisp1_capture *cap)
{
	...
	if (rkisp1_has_feature(rkisp1, MAIN_STRIDE)) {
		rkisp1_write(rkisp1, ISP3X_MI_MP_WR_Y_LLENGTH, cap->stride);
		rkisp1_write(rkisp1, ISP3X_MI_MP_WR_Y_PIC_WIDTH, pixm->width);
		rkisp1_write(rkisp1, ISP3X_MI_MP_WR_Y_PIC_HEIGHT, pixm->height);
		rkisp1_write(rkisp1, ISP3X_MI_MP_WR_Y_PIC_SIZE,
	...
	if (rkisp1->info->isp_ver == RKISP1_V30) {
		u32 mask = ISP3X_MPFBC_FORCE_UPD | ISP3X_MP_YUV_MODE;

		reg = rkisp1_read(rkisp1, ISP3X_MPFBC_CTRL) & ~mask;
		...
		rkisp1_write(rkisp1, ISP3X_MPFBC_CTRL, reg);
	}
	...
	reg |= cap->pix.cfg->write_format | RKISP1_CIF_MI_MP_AUTOUPDATE_ENABLE;
...
static void rkisp1_sp_config(struct rkisp1_capture *cap)
{
	...
	rkisp1_write(rkisp1, RKISP1_CIF_MI_SP_Y_PIC_SIZE, cap->stride * pixm->height);
...
irqreturn_t rkisp1_capture_isr(int irq, void *ctx)
{
	...
	rkisp1_dmarx_isr(rkisp1, status);
...
static bool rkisp1_rect_equals(const struct v4l2_rect *a,
...
static void rkisp1_capture_disable_dcrop(struct rkisp1_capture *cap)
{
	struct rkisp1_device *rkisp1 = cap->rkisp1;
	u32 mask = cap->config->dual_crop.yuvmode_mask |
		cap->config->dual_crop.rawmode_mask;
	u32 ctrl = rkisp1_read(rkisp1, cap->config->dual_crop.ctrl);

	ctrl &= ~mask;
	ctrl |= RKISP1_CIF_DUAL_CROP_CFG_UPD;
	rkisp1_write(rkisp1, cap->config->dual_crop.ctrl, ctrl);
}

static void rkisp1_capture_config_dcrop(struct rkisp1_capture *cap,
				      const struct v4l2_rect *crop)
{
	...
	rkisp1_write(rkisp1, cap->config->dual_crop.h_offset, crop->left);
	rkisp1_write(rkisp1, cap->config->dual_crop.v_offset, crop->top);
	rkisp1_write(rkisp1, cap->config->dual_crop.h_size, crop->width);
	rkisp1_write(rkisp1, cap->config->dual_crop.v_size, crop->height);
...
static void rkisp1_capture_disable_rsz(struct rkisp1_capture *cap)
{
	struct rkisp1_device *rkisp1 = cap->rkisp1;

	rkisp1_write(rkisp1, cap->config->rsz.ctrl, 0);
	rkisp1_write(rkisp1, cap->config->rsz.ctrl, RKISP1_CIF_RSZ_CTRL_CFG_UPD);
}

static int rkisp1_mbus_code_xysubs(u32 code, u32 *xsubs, u32 *ysubs)
{
	switch (code) {
...
static void rkisp1_format_xysubs(const struct v4l2_format_info *info,
				       u32 *xsubs, u32 *ysubs)
{
...
static u32 rkisp1_scale_up_factor(u32 src, u32 dst)
{
	if (src <= 1 || dst <= 1)
		return RKISP1_CIF_RSZ_SCALER_FACTOR;
	return ((src - 1) * RKISP1_CIF_RSZ_SCALER_FACTOR) / (dst - 1);
}

static u32 rkisp1_scale_down_factor(u32 src, u32 dst)
{
	if (src <= 1 || dst <= 1)
		return RKISP1_CIF_RSZ_SCALER_FACTOR;
	return ((dst - 1) * RKISP1_CIF_RSZ_SCALER_FACTOR) / (src - 1) + 1;
}

static void rkisp1_capture_prepare_lut(struct rkisp1_capture *cap)
{
	struct rkisp1_device *rkisp1 = cap->rkisp1;
	unsigned int i;

	for (i = 0; i < 64; i++) {
		rkisp1_write(rkisp1, cap->config->rsz.scale_lut_addr, i);
		rkisp1_write(rkisp1, cap->config->rsz.scale_lut, i);
	}
}

static void rkisp1_capture_config_rsz(struct rkisp1_capture *cap,
				      ...
				      const struct v4l2_mbus_framefmt *src_fmt)
{
	...
	rkisp1_mbus_code_xysubs(src_fmt->code, &in_xsubs, &in_ysubs);
	rkisp1_format_xysubs(cap->pix.info, &out_xsubs, &out_ysubs);
	...
	rkisp1_write(rkisp1, cap->config->rsz.phase_hy, 0);
	rkisp1_write(rkisp1, cap->config->rsz.phase_hc, 0);
	rkisp1_write(rkisp1, cap->config->rsz.phase_vy, 0);
	rkisp1_write(rkisp1, cap->config->rsz.phase_vc, 0);

	rkisp1_capture_prepare_lut(cap);
	...
	if (in_rect->width != out_rect->width) {
		...
		if (in_rect->width < out_rect->width) {
			hy = rkisp1_scale_up_factor(in_rect->width, out_rect->width);
			rsz_ctrl |= RKISP1_CIF_RSZ_CTRL_SCALE_HY_UP;
		} else {
			hy = rkisp1_scale_down_factor(in_rect->width, out_rect->width);
		}
		rkisp1_write(rkisp1, cap->config->rsz.scale_hy, hy);
	}
	...
	if (in_c.width != out_c.width) {
		...
		if (in_c.width < out_c.width) {
			hc = rkisp1_scale_up_factor(in_c.width, out_c.width);
			rsz_ctrl |= RKISP1_CIF_RSZ_CTRL_SCALE_HC_UP;
		} else {
			hc = rkisp1_scale_down_factor(in_c.width, out_c.width);
		}
		rkisp1_write(rkisp1, cap->config->rsz.scale_hcb, hc);
		rkisp1_write(rkisp1, cap->config->rsz.scale_hcr, hc);
	}
	...
	if (in_rect->height != out_rect->height) {
		...
		if (in_rect->height < out_rect->height) {
			vy = rkisp1_scale_up_factor(in_rect->height, out_rect->height);
			rsz_ctrl |= RKISP1_CIF_RSZ_CTRL_SCALE_VY_UP;
		} else {
			vy = rkisp1_scale_down_factor(in_rect->height, out_rect->height);
		}
		rkisp1_write(rkisp1, cap->config->rsz.scale_vy, vy);
	}
	...
	if (in_c.height != out_c.height) {
		...
		if (in_c.height < out_c.height) {
			vc = rkisp1_scale_up_factor(in_c.height, out_c.height);
			rsz_ctrl |= RKISP1_CIF_RSZ_CTRL_SCALE_VC_UP;
		} else {
			vc = rkisp1_scale_down_factor(in_c.height, out_c.height);
		}
		rkisp1_write(rkisp1, cap->config->rsz.scale_vc, vc);
	}

	rkisp1_write(rkisp1, cap->config->rsz.ctrl,
		rsz_ctrl | RKISP1_CIF_RSZ_CTRL_CFG_UPD);
}

static int rkisp1_capture_setup_path(struct rkisp1_capture *cap)
{
	struct rkisp1_device *rkisp1 = cap->rkisp1;
	struct v4l2_subdev *sd = &rkisp1->isp.sd;
	struct v4l2_subdev_selection sel = {
		.which = V4L2_SUBDEV_FORMAT_ACTIVE,
		.pad = RKISP1_ISP_PAD_SOURCE_VIDEO,
	};
	...
	if (rkisp1_rect_equals(&crop, &input))
		rkisp1_capture_disable_dcrop(cap);
	else
		rkisp1_capture_config_dcrop(cap, &crop);
	...
	if (!v4l2_is_format_yuv(cap->pix.info)) {
		if (crop.width != out.width || crop.height != out.height)
			return -EINVAL;
		rkisp1_capture_disable_rsz(cap);
		return 0;
	}
	...
	if (crop.width == out.width && crop.height == out.height)
		...
		rkisp1_capture_config_rsz(cap, &crop, &out, &fmt.format);
...
static void rkisp1_cap_stream_enable(struct rkisp1_capture *cap)
{
	...
	if (!has_self_path || !other->is_streaming) {
		 ...
		 * mp_output_format field in the RKISP1_CIF_MI_INIT register that must
		 * be preserved. It can be read back, but it is not clear what
		 * other register bits will return. Mask them out.
		 *
		 * On Rockchip platforms, the RKISP1_CIF_MI_INIT register is marked as
	...
	__must_hold(&cap->rkisp1->stream_lock)
{
	 ...
	 */
	if (rkisp1->pipe.start_count < (rkisp1->source ? 2 : 3))
	...
	__must_hold(&cap->rkisp1->stream_lock)
{
	...
	ret = rkisp1_capture_setup_path(cap);
	if (ret)
		return ret;
...
static void rkisp1_vb2_stop_streaming(struct vb2_queue *queue)
{
	...
	dev_dbg(cap->rkisp1->dev, "%s, enter\n", __func__);
...
{
	struct rkisp1_capture *cap = video_drvdata(file);
	struct v4l2_subdev *sd = &cap->rkisp1->isp.sd;
