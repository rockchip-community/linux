			      ...
			      const struct v4l2_subdev_state *sd_state)
{
	...
	if (isp->rkisp1->info->isp_ver == RKISP1_V30)
		return;
	...
	if (sink_fmt->pixel_enc == V4L2_PIXEL_ENC_BAYER) {
		...
		if (src_fmt->pixel_enc == V4L2_PIXEL_ENC_BAYER) {
			...
			if (rkisp1->info->isp_ver == RKISP1_V30)
				rkisp1_write(rkisp1, ISP_DEBAYER_CONTROL,
					     SW_DEBAYER_EN |
...
static void rkisp1_isp_stop(struct rkisp1_isp *isp)
{
	...
	/* Mask MIPI, MI and ISP interrupts */
	...
	for (unsigned int il = 0; il < ARRAY_SIZE(rkisp1->irqs); ++il) {
		int irq = rkisp1->irqs[il];

		if (irq < 0)
			continue;
		if (il > 0 && rkisp1->irqs[il - 1] == irq)
			continue;

		synchronize_irq(irq);
	}

	/* Clear MIPI, MI and ISP interrupt status */
...
static void rkisp1_config_clk(struct rkisp1_isp *isp)
{
	...
	} else if (rkisp1->info->isp_ver == RKISP1_V30) {
		val = RKISP1_CIF_CLK_CTRL_ISP_3A | RKISP1_CIF_CLK_CTRL_ISP_RAW;
		rkisp1_write(rkisp1, RKISP1_CIF_VI_ISP_CLK_CTRL_V12, val);

		val = RKISP1_CIF_SWS_MIPI_DROP_FRM_DIS	| RKISP1_CIF_SWS_ACK_FRM_PRO_DIS;
		rkisp1_write(rkisp1, ISP3X_SWS_CFG, val);
	}
}
			    ...
			    struct media_pad *source)
{
	...
	val |= RKISP1_CIF_ISP_CTRL_ISP_CFG_UPD |
	       RKISP1_CIF_ISP_CTRL_ISP_ENABLE |
	       RKISP1_CIF_ISP_CTRL_ISP_INFORM_ENABLE | RKISP1_CIF_ISP_CTRL_ISP_CFG_UPD_PERMANENT;
	rkisp1_write(rkisp1, RKISP1_CIF_ISP_CTRL, val);
...
static int rkisp1_isp_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (!enable) {
		if (rkisp1->source)
			v4l2_subdev_call(rkisp1->source, video, s_stream,
					 false);
		rkisp1_isp_stop(isp);
		return 0;
	}
	...
	if (is_media_entity_v4l2_subdev(source_pad->entity)) {
		...
		dev_dbg(rkisp1->dev, "ISP source is not a v4l2 subdev\n");
		rkisp1->source = NULL;
	}
