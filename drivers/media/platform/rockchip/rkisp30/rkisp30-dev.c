					...
					struct v4l2_async_connection *asc)
{
	...
	if (s_asd->port == 0) {
		dev_warn(rkisp1->dev,
			 "MIPI CSI-2 port is not supported in this configuration\n");
		return -EOPNOTSUPP;
	}
...
static int rkisp1_subdev_notifier_register(struct rkisp1_device *rkisp1)
{
	...
	fwnode_graph_for_each_endpoint(fwnode, ep) {
		...
		case 0:
			dev_err(rkisp1->dev,
				"MIPI CSI-2 port 0 is not supported\n");
			ret = -EOPNOTSUPP;
			break;
...
static int __maybe_unused rkisp1_runtime_resume(struct device *dev)
{
	...
	rkisp1_write(rkisp1, ISP3X_SWS_CFG, RKISP1_CIF_SWS_MIPI_DROP_FRM_DIS);
...
static int rkisp1_create_links(struct rkisp1_device *rkisp1)
{
	...
	for (i = 0; i < rkisp1->dmarx.num_chans; i++) {
		struct media_entity *source =
			&rkisp1->dmarx.chan[i].vnode.vdev.entity;
		if (i == RKISP1_RAWRD0) {
			ret = media_create_pad_link(
				source, 0, &rkisp1->isp.sd.entity,
				RKISP1_ISP_PAD_SINK_VIDEO,
...
{
	rkisp1_dmarx_unregister(rkisp1);
...
irqreturn_t rkisp1_mipi_isr(int irq, void *ctx)
{
	...
	struct rkisp1_device *rkisp1 = dev_get_drvdata(dev);
	struct v4l2_device *v4l2_dev = &rkisp1->v4l2_dev;
	u32 mis;

	if (!rkisp1->irqs_enabled)
		return IRQ_NONE;

	mis = rkisp1_read(rkisp1, RKISP1_CIF_MIPI_MIS);
	if (!mis)
		return IRQ_NONE;

	rkisp1_write(rkisp1, RKISP1_CIF_MIPI_ICR, mis);

	if (mis & RKISP1_CIF_MIPI_ERR_DPHY) {
		u32 mask = rkisp1_read(rkisp1, RKISP1_CIF_MIPI_IMSC);

		rkisp1_write(rkisp1, RKISP1_CIF_MIPI_IMSC, mask & ~RKISP1_CIF_MIPI_ERR_DPHY);
		rkisp1->isp.dphy_errctrl_disabled = true;
	}

	if (mis == RKISP1_CIF_MIPI_FRAME_END) {
		...
		rkisp1_write(rkisp1, RKISP1_CIF_MIPI_CTRL,
			 ...
			 RKISP1_CIF_MIPI_CTRL_FLUSH_FIFO);
	}
...
static int rkisp1_entities_register(struct rkisp1_device *rkisp1)
{
	...
	ret = rkisp1_dmarx_register(rkisp1);
	if (ret)
		goto error;
...
static irqreturn_t rkisp1_isr(int irq, void *ctx)
{
	...
	if (rkisp1_mipi_isr(irq, ctx) == IRQ_HANDLED)
		ret = IRQ_HANDLED;
...
static const struct rkisp1_isr_data px30_isp_isrs[] = {
	...
	{ "mipi", rkisp1_mipi_isr, BIT(RKISP1_IRQ_MIPI) },
};
...
static const struct rkisp1_info px30_isp_info = {
	...
	.isp_ver = RKISP1_V12,
	.features = RKISP1_FEATURE_SELF_PATH
...
static const struct rkisp1_isr_data rk3588_isp_isrs[] = {
	{ "isp_irq", rkisp1_isp_isr, BIT(RKISP1_IRQ_ISP) },
	{ "mi_irq", rkisp1_capture_isr, BIT(RKISP1_IRQ_MI) },
	{ "mipi_irq", rkisp1_mipi_isr, BIT(RKISP1_IRQ_MIPI) },
};

static const struct rkisp1_info rk3588_isp_info = {
	.clks = rk3588_isp_clks,
	.clk_size = ARRAY_SIZE(rk3588_isp_clks),
	.isrs = rk3588_isp_isrs,
	.isr_size = ARRAY_SIZE(rk3588_isp_isrs),
	.isp_ver = RKISP1_V30,
	.features = RKISP1_FEATURE_MAIN_STRIDE | RKISP1_FEATURE_SELF_PATH,
	.max_width = 4096,
	.max_height = 3072,
};
...
static int rkisp1_probe(struct platform_device *pdev)
{
	...
	rkisp1->has_iommu = is_iommu_enable(dev);
